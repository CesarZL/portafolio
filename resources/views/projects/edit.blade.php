<x-app-layout>
    <div class="max-w-screen-lg mx-auto px-5 mt-14 mb-20">
        <div class="bg-white p-5 md:p-8 rounded-xl mb-5 shadow-lg dark:bg-gray-800 dark:text-gray-300">
            <div class="pb-10 h-3/6">
                <img id="preview-image" src="{{ asset('storage/' . $project->image) }}" alt="Imagen actual" class="mt-2 w-full h-96 object-cover rounded-lg">
            </div>
            <form action="{{ route('projects.update', $project) }}" method="POST" novalidate enctype="multipart/form-data" id="ProjectForm">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="mb-2 col-span-2 sm:col-span-1">
                        ...
                        <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title', $project->title)" placeholder="Título del proyecto en español" required />
                    ...
                    <div class="mb-2 col-span-2 sm:col-span-1">
                        ...
                        <x-text-input id="title_en" class="block mt-1 w-full" type="text" name="title_en" :value="old('title_en', $project->title_en)" placeholder="Título del proyecto en inglés" required />
                    ...
                    <div class="mb-2 col-span-2 sm:col-span-1">
                        ...
                        <x-file id="image" type="file" name="image" class="mt-1 block w-full" accept="image/*" required onchange="previewImage(event)"  />
                    ...
                    <div class="mb-2 col-span-2 sm:col-span-1">
                        ...
                        <x-select id="principal_category" class="block mt-1 w-full" name="principal_category" required>
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ old('principal_category', $project->category->id) == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </x-select>
                    ...
                    <div class="mb-2 col-span-2">
                        ...
                        <div id="editor" class="">{!! old('content', $project->content) !!}</div>
                    ...
                    <div class="mb-2 col-span-2">
                        ...
                        <div id="editor_en" class="">{{old('content', $project->content_en)}}</div>
                ...
                <div class="flex justify-end mt-8 gap-x-4">
                    <x-secondary-button class="w-full md:w-1/6 flex items-center justify-center" href="{{ route('projects.index') }}">
                        {{ __('Cancelar') }}
                    </x-secondary-button>
